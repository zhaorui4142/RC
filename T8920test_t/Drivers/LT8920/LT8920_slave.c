...
*功能说明: //主机发送配对请求
*******************************************************************************/
bool LT8920_PairingRequest(uint8_t ch, uint32_t waiting_ms)
{
    //记录原先使用的频道
    uint8_t savedCH = TxRx_CH;
    ...
    buf[0] = DeviceID;
    buf[1] = savedCH;//发送使用的频道
    LT8920_Transmit(DeviceID, FUN_PAIR_REQUEST, buf, 100);
    
    //等待从机回应
    uint8_t respones, receiverID;
    bool return_value = false;
    if(LT8920_Receive(&receiverID, &respones, buf, waiting_ms))
    {
...
*功能说明: //从机等待配对
*******************************************************************************/
bool LT8920_PairingWaiting(uint8_t pair_ch, uint8_t* nomal_ch, uint32_t waiting_ms)
{
    //设定频道
    LT8920_SetChannel(pair_ch);
    ...
    uint8_t remoterID;
    bool return_value = false;
    if(LT8920_Receive(&remoterID, &cmd, buf, waiting_ms))
    {
        if(cmd == FUN_PAIR_REQUEST)
        {
            //回应广播数据包
